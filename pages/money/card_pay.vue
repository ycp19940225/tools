	...
	<view class="app">
		<view class="price-box" style="display: flex;flex-direction: column;background-color: #fff;">
			<view class="price_box" >
				<view class="price_item" v-for="(item, index) in cardList" :key="index" :class="{active: card_id == index}" @click="getCardId(index)">
					<text class="text-price">{{item.price}}</text>
				</view>
			</view>
			
		</view>
		<view class="order_box" style="display: flex;flex-direction: row;justify-content: space-around;">
			<view class="order_title">
				<text class="text-black text-bold">请选择数量</text>
			</view>
		...
		methods: {
			...
			calculateTotol(){
				...
				if(this.cardList.length > 0){
					this.totalFee = (card.price*1*this.choose_number).toFixed(2);
				}
			},
			checkPay(id) {
				var _this = this;
