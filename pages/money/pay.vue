	...
	<view class="app">
		...
		<view class="order_box">
			...
			<view class="order_detail">
				<view>
					<text class="text-grey">产品类型:</text><text class=" text-df padding">活动报名</text>
				</view>
				<view>
					<text class="text-grey">订单编号:</text><text class=" text-df padding">{{orderInfo.order_sn}}</text>
				</view>
				<view>
					<text class="text-grey">赛事名称:</text><text class=" text-df padding">{{orderInfo.productName}}</text>
				</view>
				<view>
					<text class="text-grey">组别选择:</text><text class=" text-df padding">{{orderInfo.groupName}}</text>
				</view>
				<view v-if="orderInfo.packageName">
					<text class="text-grey">套餐选择:</text><text class="text-df padding">{{orderInfo.packageName}}</text>
				</view>
				<view v-if="orderInfo.productData != undefined">
					<text class="text-grey">服务选择:</text>
					<text v-for="(itemData,index) in orderInfo.productData" :key="index" class="padding">
						{{itemData.name}}({{itemData.attr}})
					</text>
				</view>
			</view>
		</view>
		<view class="pay-type-list">
			...
			<view class="type-item" @click="changePayType(3)">
				...
				<view class="con">
					...
					<text>可用余额 <text class="text-price">{{amount}}</text></text>
				</view>
				<label class="radio">
		...
		<text class="mix-btn text-green" @click="confirm"> 确认支付</text>
	</view>
</template>
		...
		methods: {
			calculateTotol(key){
				if(key != -1){
					...
					this.totalFee = (this.orderInfo.price - coupons.reduced_price).toFixed(2);
					this.selectTitle = '选择:' + coupons.name + '  优惠金额:￥' + coupons.reduced_price
				}else{
					this.totalFee = (this.orderInfo.price * 1).toFixed(2)
			...
			checkPay(id) {
				...
				this.$http.post('/api/order/checkPay', data).then(res => {
					console.log(res)
					if (res.data.code != 1) {
						uni.showToast({
							title: res.data.info,
			...
			getPageData(data) {
				...
				this.$http.post('/api/order/getOrder', data).then(res => {
					console.log(res)
					if (res.data.code != 1) {
						uni.showToast({
							title: res.data.info,
						...
						_this.totalFee = _this.orderInfo.price;
			...
			confirm: function() {
				var _this = this;
				if (_this.payType == 3 && _this.amount == 0) {
					uni.showToast({
						title: '余额不足',
						icon: 'none'
					});
				...
				this.$http.post('/api/order/doPay', data).then(res => {
					...
					} else if(res.data.code == 19 || res.data.code == 29){ //0元或者 储值支付
						//订单支付成功
