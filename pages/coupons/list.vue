				...
				<view class="coupon-detail bg-white">
					<view class="flex  align-center">
						<view v-if="item.type == 1" class="padding">
							...
							<view class="end_time" v-if="item.limit_use_type == 3">有效期:无限制</view>
				...
				<view class="coupon-money padding light bg-red ">
					<view class="flex flex-direction justify-center">
						<view v-if="item.type == 1">
							...
							<view class=" text-sm padding text-center" v-if="item.discount_value != -1 && item.discount_value > 0">
						...
						<view v-if="item.type == 2">
							...
							<view class=" text-sm padding text-center" v-if="item.meet_value != -1 && item.reduction > 0">
						...
						<view v-if="item.type == 3">
							...
							<view class=" text-sm padding text-center" >
								<text class="text-red">无门槛</text>
							</view>
						</view>
						<view class="cu-btn bg-gray sm shadow padding-sm round" v-if="item.status == 3">
							已过期
						</view>
						<view class="cu-btn bg-gray sm shadow padding-sm round" v-if="item.status == 2">
							已使用
						</view>
						<view class="cu-btn bg-red sm shadow padding-sm round" @click="goProduct(item.use_type)" v-if="item.status == 1">
							立即使用
						</view>
					</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			getPageData() {
				var _this = this;
				this.$http.post('/api/coupons/myCoupons', {}).then(res => {
					if (res.data.code != 1) {
						uni.showToast({
							title: res.data.info,
							icon: 'none'
