				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<view v-for="(item,index) in tabItem.orderList" :key="index" class="order-item">
						...
						<view class="goods-box-single">
							...
							<view class="right">
								<text class="title clamp attr-box">订单编号：{{item.order_sn}}</text>
								<!-- <text class="attr-box">item.activityInfo.name</text> -->
								<text class="price attr-box">开赛时间：{{item.start_time}}</text>
							</view>
						</view>
						...
						<view class="action-box b-t">
							...
							<button class="action-btn recom" @click="editInfo()" v-if="item.state == 3">完善资料</button>
	...
	export default {
		...
		methods: {
			...
			loadData(source) {
				...
				// navItem.tabCurrentPage = page;
				let page;
				navItem.tabCurrentPage != undefined ? page = navItem.tabCurrentPage : page = 1;
				// console.log()
				...
				let data = {
					'status': state,
					'pageNum': navItem.tabCurrentPage != undefined ? navItem.tabCurrentPage : 1
				}
				this.$http.post('/api/order/order_list', data).then(res => {
					console.log(res)
					if (res.data.code != 1) {
						uni.showToast({
							title: res.data.info,
						...
						setTimeout(() => {
							...
							if (page < pageInfo.pageSize) {
								...
								navItem.tabCurrentPage = page + 1;
							} else {
								navItem.loadingType = 'noMore';
							}

							console.log(page, _this.tabCurrentPage, navItem)
						}, 600);
					}
				}).catch(err => {
					console.log(err);
