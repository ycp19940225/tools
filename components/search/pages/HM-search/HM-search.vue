...
<script>
	...
	import mSearch from '@/components/search/components/mehaotian-search-revision/mehaotian-search-revision.vue';
	export default {
		data() {
			return {
				defaultKeyword: "",
		...
		methods: {
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['马拉松', '北京', '上海', '西安'];
			}, 
			//监听输入
			inputChange(event) {
				//兼容引入组件时传入参数情况
			...
			doSearch(key) {
				...
				uni.redirectTo({
					url: '/pages/search/select?keyword=' + key
				});
...
<style>
	view{display:block;}
	.search-box {width:95%;background-color:rgb(242,242,242);padding:15upx 2.5%;display:flex;}
	.search-box .mSearch-input-box{width: 100%;margin-left: 1.5em;}
