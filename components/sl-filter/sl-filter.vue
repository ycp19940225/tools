		...
		<view :style="{height: tabHeight + 1 +'px'}">
			<view :class="topFixed?'select-tab-fixed-top':'select-tab'" :style="{height: tabHeight+'px'}">
				...
				<view class="search" @click="goSearch()">
					<uni-text class="lg text-gray cuIcon-search"><span></span></uni-text><text class="serach_text">搜索</text>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			setSelect(option) {
				...
				if (option) {
					...
					for (let i = 0; i < this.menuList.length; i++) {
						if (i == 0) {
							if (option.cate) {
								this.titleList[i].title = option.cate.title;
							}
						}
						if (i == 1) {
							if (option.city) {
								this.titleList[i].title = option.city.title;

							}
						}
						if (i == 2) {
							if (option.date) {
								if (option.date.title != -1) {
									this.titleList[i].title = option.date.title;
								}
							}
						}
					}
					//调用子组件设置选中
					this.$refs.slFilterView.setDefaultVal(option);
				}
			},
			goSearch() {
				uni.navigateTo({
			...
			showMenuClick(index) {
				...
				this.$refs.slFilterView.clearItemList();
